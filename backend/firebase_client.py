...
class FirebaseClient:
        ...
        if HAS_FIREBASE:
            ...
            try:
                if os.path.exists(cred_path):
                    ...
                    # Attempt application default credentials initialization
                ...
                # Log or handle warning and fallback to offline mode
                pass
    ...
    def create_incident(self, incident_id: str, data: Dict[str, Any]) -> None:
        ...
        if self.offline_mode:
            ...
            try:
                self.firestore_client.collection("incidents").document(incident_id).set(data)
            except Exception:
                # Fallback to local memory on write failure
                self._mock_incidents[incident_id] = data
                self._mock_streams[incident_id] = []

    def get_incident(self, incident_id: str) -> Optional[Dict[str, Any]]:
        """Fetch incident document from Firestore."""
        if self.offline_mode or incident_id not in self._mock_incidents:
            if not self.offline_mode:
                try:
                    doc = self.firestore_client.collection("incidents").document(incident_id).get()
                    if doc.exists:
                        return doc.to_dict()
                except Exception:
                    pass
    ...
    def update_incident(self, incident_id: str, data: Dict[str, Any]) -> None:
        """Update fields in an existing incident document."""
        if self.offline_mode or incident_id not in self._mock_incidents:
            if not self.offline_mode:
                try:
                    self.firestore_client.collection("incidents").document(incident_id).update(data)
                    return
                except Exception:
                    pass
    ...
    def push_reasoning_log(self, incident_id: str, log_message: str) -> None:
        """Push a live agent reasoning log to Realtime Database stream."""
        timestamp = time.time()
        ...
        if self.offline_mode:
            ...
            try:
                self.db_client.child("streams").child(incident_id).push(log_entry)
            except Exception:
                if incident_id not in self._mock_streams:
                    self._mock_streams[incident_id] = []
                self._mock_streams[incident_id].append(log_entry)
    ...
    def get_reasoning_logs(self, incident_id: str) -> list:
        """Retrieve reasoning logs for a given incident."""
        if self.offline_mode or incident_id not in self._mock_streams:
            if not self.offline_mode:
                try:
                    res = self.db_client.child("streams").child(incident_id).get()
                    if res:
    ...
    def get_forecast(self, zone_id: str) -> Dict[str, Any]:
        ...
        # Standard mock data matching typical zone metrics
        return {
            "zone_id": zone_id,
