...
async def main():
    ...
    if not os.path.exists(scenarios_path):
        print(f"Error: Scenarios file not found at {sys.argv[1]} or resolved path {scenarios_path}")
        return
    ...
    for idx, scenario in enumerate(scenarios[:3], 1):
        ...
        base_eta = scenario.get('base_eta_to_destination', scenario.get('baseline_eta_minutes', 22))
