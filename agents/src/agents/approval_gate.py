...
class ApprovalGateAgent(Agent):
    ...
    def _init_db_client(self):
        ...
        try:
            ...
            logger.warning("FirebaseClient not available; approval gate will use mock responses")
            self.db = None
    ...
    async def execute(
        ...
        logger.info(f"[ApprovalGate] Evaluating incident {i_id} (impact_score={impact:.2f})")
        
        # 1. Check for auto-approval (low-impact incidents)
        if impact < AUTO_APPROVE_THRESHOLD:
            logger.info(f"[ApprovalGate] Auto-approving: impact_score {impact:.2f} below threshold {AUTO_APPROVE_THRESHOLD}")
            if self.db:
                self.db.update_incident(i_id, {"status": "executing"})
                self.db.push_reasoning_log(i_id, f"[ApprovalGate] Auto-approved: impact below threshold ({impact:.2f} < {AUTO_APPROVE_THRESHOLD})")
            return {
        ...
        if demo_mode:
            logger.info(f"[ApprovalGate] Demo mode: auto-approving after {DEMO_AUTO_APPROVE_DELAY}s delay")
            if self.db:
                self.db.update_incident(i_id, {"status": "pending_approval"})
                self.db.push_reasoning_log(i_id, "[ApprovalGate] Awaiting operator approval (demo mode)...")
            
            await asyncio.sleep(DEMO_AUTO_APPROVE_DELAY)
            
            if self.db:
                self.db.update_incident(i_id, {"status": "executing"})
                self.db.push_reasoning_log(i_id, "[ApprovalGate] Demo auto-approved. Proceeding to execution.")
            return {
                "status": "approved",
            ...
            if self.db:
                ...
                if incident:
                    ...
                    if current_status in ("executing", "denied"):
                        ...
                        op_reason = operator_decision.get("reason", "Operator decision recorded")
                        
                        logger.info(f"[ApprovalGate] Operator decision received: {op_status} ({elapsed:.1f}s)")
                        self.db.push_reasoning_log(
                            i_id,
                            f"[ApprovalGate] Operator {op_status}. Wait time: {elapsed:.1f}s. Reason: {op_reason}"
                        )
                        return {
                            "status": "approved" if current_status == "executing" else "denied",
                            "reason": op_reason,
                            "wait_time_seconds": elapsed
                        }
            
            logger.debug(f"[ApprovalGate] Still waiting for approval... ({elapsed:.1f}s elapsed)")
        
        # 4. Timeout — no operator response
        logger.warning(f"[ApprovalGate] Timeout after {APPROVAL_TIMEOUT_SECONDS}s. No operator response.")
        if self.db:
            self.db.update_incident(i_id, {"status": "timeout"})
            self.db.push_reasoning_log(i_id, f"[ApprovalGate] TIMEOUT: No operator response after {APPROVAL_TIMEOUT_SECONDS}s")
