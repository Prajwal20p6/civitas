...
try:
    ...
    try:
        ...
        class FirebaseClient:
            def create_incident(self, incident_id, data):
                pass

            def update_incident(self, incident_id, data):
                pass

            def push_reasoning_log(self, incident_id, msg):
                pass
...
class OrchestratorAgent(SequentialAgent):
    ...
    async def execute(
        self, incident: IncidentInput, incident_id: str = None
    ) -> OrchestratorDecision:
        """
        ...
        i_id = (
            incident_id
            or getattr(incident, "incident_id", None)
            or f"incident_{uuid.uuid4().hex[:8]}"
        )
        ...
        self.db.push_reasoning_log(
            i_id, "[Orchestrator] Running Perception Agent classification..."
        )
        ...
        self.db.push_reasoning_log(
            i_id,
            f"[Perception] Classified type: {classification.incident_type}, severity: {classification.severity}, priority: {classification.priority_score}",
        )
        ...
        self.db.push_reasoning_log(
            i_id, "[Orchestrator] Spawning Route Agents A & B in parallel..."
        )
        import asyncio
        ...
        self.db.push_reasoning_log(
            i_id,
            f"[Route Agent A] Recommends: {proposal_a.recommended_route} (ETA: {proposal_a.ambulance_eta} min)",
        )
        self.db.push_reasoning_log(
            i_id,
            f"[Route Agent B] Recommends: {proposal_b.recommended_route} (ETA: {proposal_b.ambulance_eta} min)",
        )
        ...
        self.db.update_incident(
            i_id, {"negotiation_result": negotiation_res.model_dump()}
        )
        self.db.push_reasoning_log(
            i_id,
            f"[Simulation] Resolved winner: {negotiation_res.winner} (Score: {negotiation_res.score_a} vs {negotiation_res.score_b})",
        )

        # Step 4: Generate Explainability natural briefs
        self.db.push_reasoning_log(
            i_id, "[Orchestrator] Compiling decision explanation..."
        )
        ...
        self.db.push_reasoning_log(
            i_id,
            f"[Explainability] operator Brief: {explain_output.reasoning_one_liner}",
        )

        # Set initial approval status
        self.state.write(
            "approval_status",
            "pending" if explain_output.approval_required else "approved",
        )
