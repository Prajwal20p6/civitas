...
async def test_auto_approval_low_impact(mock_db, sample_gate_input):
    ...
    # Verify DB updates
    mock_db.update_incident.assert_called_once_with("test_incident_123", {"status": "executing"})
...
async def test_demo_mode_auto_approval(mock_db, sample_gate_input):
    ...
    with patch.dict("os.environ", {"CIVITAS_DEMO_MODE": "true"}), \
         patch("src.agents.approval_gate.DEMO_AUTO_APPROVE_DELAY", 0.01), \
        ...
        # Verify DB updates
        mock_db.update_incident.assert_any_call("test_incident_123", {"status": "pending_approval"})
        mock_db.update_incident.assert_any_call("test_incident_123", {"status": "executing"})
...
async def test_live_operator_approved(mock_db, sample_gate_input):
    ...
    with patch.dict("os.environ", {"CIVITAS_DEMO_MODE": "false"}), \
         patch("src.agents.approval_gate.POLL_INTERVAL_SECONDS", 0.01), \
        ...
        # Verify DB updates
        mock_db.update_incident.assert_called_once_with("test_incident_123", {"status": "pending_approval"})
...
async def test_live_operator_timeout(mock_db, sample_gate_input):
    ...
    with patch.dict("os.environ", {"CIVITAS_DEMO_MODE": "false"}), \
         patch("src.agents.approval_gate.POLL_INTERVAL_SECONDS", 0.01), \
         patch("src.agents.approval_gate.APPROVAL_TIMEOUT_SECONDS", 0.02), \
         patch("asyncio.sleep", return_value=None):
        ...
        mock_db.update_incident.assert_any_call("test_incident_123", {"status": "timeout"})
